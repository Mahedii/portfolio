<script type="text/javascript">
    ...
    $(document).ready(function () {
         ...
         */
        $('#skillsListsData').on('click', '.ajax-edit-data-btn', function () {
            ...
            $.ajax({
                ...
                success:function(data){
                    $('#skills_secret_key').val(data.table_secret_key);
                    $('#skills-row-id').val("row-"+id);
                    var tableData = data.field;
                    tableData.forEach(function(row) {
                        $('#slug').val(row.slug);
                        $('#skillNameVal').val(row.skill_name);
         ...
         */
        $('#skillsListsData').on('click', '.ajax-delete-data-btn', function () {
            var slug = $(this).data('slug');
            var table_secret_key = $(this).closest('tr').data('table-secret');
        ...
        /**
         * Add skills lists data
         */
        $(document).on("submit", "#skillsListsAddForm", function(e){

            e.preventDefault();

            $('#skillsListsAddForm .ajax-submit .submit-btn-text').toggleClass('hide');
            $('#skillsListsAddForm .ajax-submit .ajax-spinner').toggleClass('hide');
            var classNameOrId = "#skillsListsAddForm";

            var dataString = $('#skillsListsAddForm').serialize();
            // alert(dataString);
         ...
         */
        $(document).on("submit", "#skillsListsUpdateForm", function(e){

            e.preventDefault();

            $('#skillsListsUpdateForm .ajax-submit .submit-btn-text').toggleClass('hide');
            $('#skillsListsUpdateForm .ajax-submit .ajax-spinner').toggleClass('hide');
            var classNameOrId = "#skillsListsUpdateForm";

            var form = $('#skillsListsUpdateForm')[0];
        ...
        /**
         * Delete selected data for skills lists
         */
        ...
        $(document).on("submit", "#skillsUpdateForm", function(e){
            ...
            $('#skillsUpdateForm .ajax-submit .submit-btn-text').toggleClass('hide');
            $('#skillsUpdateForm .ajax-submit .ajax-spinner').toggleClass('hide');
            var classNameOrId = "#skillsUpdateForm";

            var form = $('#skillsUpdateForm')[0];
            let formData = new FormData(form);
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.status == 200) {
                        ...
                        $(".skills_title").val(data.updatedRowData[0].title);
                        $(".skills_title_description").val(data.updatedRowData[0].title_description);
