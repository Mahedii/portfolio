<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        /**
         * Fetch selected inputs information for social media
         * Set them in a modal for update
         */
        $('#socialMediaData').on('click', '.ajax-edit-data-btn', function () {
            ...
            $.ajax({
                ...
                success:function(data){
                    $('#social_media_secret_key').val(data.table_secret_key);
                    $('#social-media-row-id').val("row-"+id);
                    var tableData = data.field;
                    tableData.forEach(function(row) {
                        $('#slug').val(row.slug);
                        $('#socialMediaNameVal').val(row.name);
                        $('#socialMediaUrlVal').val(row.url);
         ...
         */
        $('#socialMediaData').on('click', '.ajax-delete-data-btn', function () {
            var slug = $(this).data('slug');
            var table_secret_key = $(this).closest('tr').data('table-secret');
        ...
        /**
         * Add social media lists data
         */
        $(document).on("submit", "#socialMediaAddForm", function(e){

            e.preventDefault();

            $('#socialMediaAddForm .ajax-submit .submit-btn-text').toggleClass('hide');
            $('#socialMediaAddForm .ajax-submit .ajax-spinner').toggleClass('hide');
            var classNameOrId = "#socialMediaAddForm";

            var dataString = $('#socialMediaAddForm').serialize();
            // alert(dataString);
        ...
        /**
         * Update social media lists data
         */
        $(document).on("submit", "#socialMediaUpdateForm", function(e){

            e.preventDefault();

            $('#socialMediaUpdateForm .ajax-submit .submit-btn-text').toggleClass('hide');
            $('#socialMediaUpdateForm .ajax-submit .ajax-spinner').toggleClass('hide');
            var classNameOrId = "#socialMediaUpdateForm";

            var form = $('#socialMediaUpdateForm')[0];
            let formData = new FormData(form);
            var rowId = $("#social-media-row-id").val();
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.status == 200) {
                        ...
                        row.cell(row.index(), 1).data(updatedRowData[0].name); // Replace 'text' with the appropriate field name
                        row.cell(row.index(), 2).data(updatedRowData[0].url);
                        row.cell(row.index(), 3).data(updatedRowData[0].icon_name);
                        row.cell(row.index(), 4).data(createActions(updatedRowData[0]));
                        row.draw(false).node();
        ...
        /**
         * Delete selected data for social media lists
         */
        ...
        function showData(data){
            ...
            $.each(data, function( index, value ) {
                var row = table.row.add([id++, value.name, value.url, value.icon_name, createActions(value)]).draw(false).node();
